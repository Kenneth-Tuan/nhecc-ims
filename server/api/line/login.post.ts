export default defineEventHandler(async (event) => {
  const body = await readBody<{ idToken?: string; accessToken?: string }>(
    event
  );
  const config = useRuntimeConfig();

  if (!config.lineChannelId) {
    console.error("lineChannelId is not set in runtimeConfig");
    throw createError({
      statusCode: 500,
      statusMessage: "Server configuration error: lineChannelId is missing",
    });
  }
